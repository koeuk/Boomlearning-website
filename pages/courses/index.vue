  ...
  <div>
    ...
    <div class="bg-white border-b border-gray-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Explore Courses</h1>
        <p class="text-gray-500 mt-1">Find the perfect course to level up your skills</p>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Results info -->
      <div v-if="courses.length > 0 && status !== 'pending'" class="mt-6 mb-4">
        <p class="text-sm text-gray-400">
          Showing {{ courses.length }} course{{ courses.length === 1 ? '' : 's' }}
          <template v-if="pagination">of {{ pagination.total }}</template>
        </p>
      </div>
      ...
      <!-- Pagination -->
      <div v-if="pagination && pagination.total_pages > 1" class="mt-10 flex items-center justify-center gap-2">
